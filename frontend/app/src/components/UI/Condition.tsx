...
export const Condition = ({ node, setMinimize, minimize, setEdited, edited }: { node: BaseNode, setMinimize: (minimize: boolean) => void, minimize: boolean, setEdited: (data: boolean) => void, edited: boolean })=>{
    ...
    const [initialConditionConfig, setInitialConditionConfig] = useState<conditionConfigType>(conditionConfig)
   ...
   return <>{minimize ? <div className="space-y-3">
             <div key={conditionConfig.id} className="flex gap-2 items-center">
                                 <Input value={conditionConfig.id} onChange={(value) => updateRow(type, item.id, "id", value)} placeholder="Key" />
                                 <Input value={conditionConfig.rule} onChange={(value) => updateRow(type, item.id, "name", value)} placeholder="Name" />      
                                 <Input value={conditionConfig.if_true} onChange={(value) => updateRow(type, item.id, "path", value)} placeholder="Path" />
                                 <Input value={conditionConfig.if_false} onChange={(value) => updateRow(type, item.id, "path", value)} placeholder="Path" />
            </div>
